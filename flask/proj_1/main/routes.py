...
def Topic(playlist_id=None, topic=None):
    ...
    if form.validate_on_submit():
        try:
            ...
            pass
        post = Post(title='test', content=form.content.data, user_id=current_user.id,
    ...
    if request.method == 'GET':
        ...
        if access_token:
            ...
            try:
                ...
                try:
                    user = User(spotify_id=me['id'], username=me['display_name'], email=me['email'],
                                image_url=me['images'][0]['url'], access_token=access_token)
                    db.session.rollback()
                    db.session.add(user)
                    db.session.commit()
                except:
                    user = User(spotify_id=me['id'], username=me['display_name'],
                                email=me['email'], access_token=access_token)
                    db.session.rollback()
                    db.session.add(user)
                    db.session.commit()
                login_user(user, remember=True)
...
def Song_data():
    ...
    if int(request.headers['Counter']) == 0:
        spotify.current_playlist = {
            'local_id': int(request.headers['Playlist-ID'])}
...
def Cached_playlist_data(playlist_id=None):
    if current_user.is_authenticated:
        ...
        try:
            ...
            spotify.call_playlists(current=int(
                request.headers['Counter']), next=5)
        except Exception as e:
            # print(e)
            pass
...
def Cached_song_data(playlist_id=None):
    if current_user.is_authenticated:
        ...
        if int(request.args.get('count')) <= 100:
            spotify.call_songs(request.args.get('user_id'), request.args.get('playlist_id'), current=int(
                request.args.get('start')), next=int(request.args.get('count')))
        else:
            for counter in range(0, int(request.args.get('count')), 100):
                print(counter)
                spotify.call_songs(request.args.get('user_id'), request.args.get(
                    'playlist_id'), current=counter, next=100)  # max range for api hardcoded as 100
        ...
        spotify.set_user_current_playlist(request.args.get(
            'user_id'), request.args.get('playlist_id'))

...
class SpotifyApi():
    ...
    def __init__(self):
        ...
        # '.spotipyoauthcache' if disabled no cache generated at backend ( no cache needed if you store token at DB == no cache management :) )
        self.CACHE = None
        self.active = None
        self.playlists = None
        self.songs = None
    ...
    def call_playlists(self, current=0, next=5):
        if self.playlists == None or len(self.playlists['items']) < next:
            # limit max 50 need to offset after 50 for next 50
            self.playlists = self.active.current_user_playlists(
                limit=next, offset=current)
            for playlist in range(0, len(self.playlists['items'])):
                # need refactor (green) issue with overwritten genre objects
                self.playlists['items'][playlist].update({'genres': []})
    ...
    def call_features(self):
        ...
        for song in range(0, len(self.songs['items'])):
            list_of_tracks.append(
                str(self.songs['items'][song]['track']['id']))

        for batch in range(0, len(list_of_tracks), 100):
            self.features += self.active.audio_features(
                tracks=list_of_tracks[batch:batch+100])
    ...
    def call_tags(self, spotify_playlist_id):
        ...
        for song in range(0, len(self.songs['items'])):
            if len(track_ids) < 50:  # hardcoded limit for tag lookup related artist API
                track_ids.add(self.songs['items'][song]
        ...
        if genres:
            genres = sorted(
                genres.items(), key=lambda genres: genres[1], reverse=True)
    ...
    def insert_tag(self):
        tag_to_db = Playlist.query.filter_by(
            spotify_id=self.current_playlist['id']).first()
    ...
    def insert_current_playlist(self, local_user_id):
        ...
        playlist_from_db = Playlist.query.filter_by(
            spotify_id=playlist_to_db.spotify_id).first()
        # print(playlist)
        if playlist_from_db != None:
            pass
        else:
    ...
    def insert_songs(self, playlist_id):
        ...
        for song in range(0, len(self.songs['items'])):
            song_to_db = Song(order=song,
                              ...
                              # alternative-danceability = self.features[song]['features']['danceability'],
                              danceability=self.songs['items'][song]['features']['danceability'],
            ...
            if song_from_db != None:
                ...
                if song_from_db != None:
                    if song_from_db.spotify_id != song_to_db.spotify_id:
                        ...
                        try:
                            Song.query.filter(
                                Song.id >= song_from_db.id).delete()
                            db.session.commit()
    ...
    def set_user_current_playlist(self, spotify_user_id, spotify_playlist_id):
        self.current_playlist = self.active.user_playlist(
            spotify_user_id, playlist_id=spotify_playlist_id, fields=None)

...
class FilterIntsTestCase(TestCase): # 5
    ...
    def test_is_positive(self):
        assert_equal(False, is_positive(-1)) # before boundary with granularity 1
        assert_equal(False, is_positive(0)) # on the boundary with granularity 1
        assert_equal(True, is_positive(1)) # after the boundary with granularity 1

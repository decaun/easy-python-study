...
class pyRBT(object):
    ...
    class RBNode(object):
        ...
        def treestr(self, showpa=False):
            if showpa:
                pa = "[" + (str(self.parent.value)
                            if self.parent is not None else "-1") + "]"
            else:
                pa = ""
    ...
    class RBTIterator(object):
        ...
        def __init__(self, tree, reverse=False, nxt=None):
            ...
            if nxt is None and not tree.root.isleaf():
                nxt = tree.root
                if not reverse:
                    while not nxt.l.isleaf():
                        nxt = nxt.l
                else:
                    while not nxt.r.isleaf():
                        nxt = nxt.r
            self.nxt = nxt
        ...
        def next_node(node, tree, fwd, nxt=None):
            if node is None:
                ...
                node = node.r
                while not node.l.isleaf():
                    node = node.l
            elif not fwd and not node.l.isleaf():
                # Take the secondary fork (left fork when reverse)
                node = node.l
                while not node.r.isleaf():
                    node = node.r
            else:
                # Back up the tree - find first parent node that used left node
                if fwd:
                    while node.parent is not None and node is node.parent.r:
                        node = node.parent
                else:
                    while node.parent is not None and node is node.parent.l:
                        node = node.parent
                node = node.parent
            return node
        ...
        def __next__(self):
            self.node = pyRBT.RBTIterator.next_node(
                self.node, self.tree, self.fwd, self.nxt)
        ...
        def delete(self):
            self.nxt = pyRBT.RBTIterator.next_node(
                self.node, self.tree, self.fwd, self.nxt)
    ...
    class RBTValIterator(RBTIterator):
        def __next__(self): return super(
            pyRBT.RBTValIterator, self).__next__().value
        def __prev__(self): return super(
            pyRBT.RBTValIterator, self).__prev__().value
    ...
    def __hash__(self):
        ...
        for v in self:
            h = ((h*33) ^ hash(v)) & 0xffffffffffffffff
        return h
    ...
    def insert(self, item, multiset=False):
        ...
        """
        if len(self) == 0:
            newv = self.root = pyRBT.RBNode(item)
        else:
    ...
    def _delete_node(self, node):
        ...
        if adjnode is not node:
            self._swap_nodes(adjnode, node)
        for v in node.path():
            v.size -= 1
    ...
    def getnode(self, i, start=None):
        ...
        if i < 0:
            i += len(node)  # allow negative indices
        if i < 0 or i >= len(node):
            raise IndexError("index out of range (%d vs 0..%d)" %
                             (i, len(node)))
        while not node.isleaf():
            if i < len(node.l):
                node = node.l
    ...
    def index(self, item, start=None):
        ...
        while not node.isleaf():
            if item < node.value:
                ...
                node = node.r
        if idx is None:
            raise KeyError('Key not found: '+str(item))
        return idx
    ...
    def check(self):
        ...
        assert (len(self) == 0) == self.root.isleaf(
        )  # size is zero only if empty
